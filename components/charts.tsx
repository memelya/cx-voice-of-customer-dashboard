...
export function PriorityMatrix() {
  ...
  return <ResponsiveContainer width="100%" height={310}><ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 0 }}><CartesianGrid stroke="rgba(15,23,42,.1)"/><XAxis dataKey="complexity" name="Complexity" tick={{ fill: "#64748b", fontSize: 12 }}/><YAxis dataKey="urgency" name="Urgency" tick={{ fill: "#64748b", fontSize: 12 }}/><ZAxis dataKey="z" range={[120, 520]}/><Tooltip cursor={{ strokeDasharray: "3 3" }} contentStyle={{ borderRadius: 14 }}/><Scatter data={data} fill="#0f766e"/></ScatterChart></ResponsiveContainer>;
}

export function ScenarioChart() {
  return <ResponsiveContainer width="100%" height={280}><AreaChart data={scenarios} margin={{ left: -10, right: 12 }}><defs><linearGradient id="scenario" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2563eb" stopOpacity={.28}/><stop offset="95%" stopColor="#2563eb" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="rgba(15,23,42,.08)" vertical={false}/><XAxis dataKey="scenario" tick={{ fill: "#475569", fontSize: 11 }} axisLine={false} tickLine={false}/><YAxis tick={{ fill: "#64748b", fontSize: 12 }} axisLine={false} tickLine={false}/><Tooltip contentStyle={{ borderRadius: 14 }}/><Area dataKey="nps" stroke="#2563eb" fill="url(#scenario)" strokeWidth={3}/><Line dataKey="retention" stroke={teal} strokeWidth={2}/></AreaChart></ResponsiveContainer>;
}

export function SentimentTrend() {
  return <ResponsiveContainer width="100%" height={260}><AreaChart data={cxHistory}><CartesianGrid stroke="rgba(255,255,255,.07)" vertical={false}/><XAxis dataKey="period" tick={{ fill: "#94a3b8", fontSize: 11 }} axisLine={false} tickLine={false}/><YAxis hide/><Tooltip contentStyle={{ background: "#09111d", border: "1px solid rgba(255,255,255,.12)", borderRadius: 14 }}/><Area type="monotone" dataKey="app" stroke={blue} fill="rgba(96,165,250,.16)" strokeWidth={3}/><Area type="monotone" dataKey="churnRisk" stroke={amber} fill="rgba(245,158,11,.12)" strokeWidth={2}/></AreaChart></ResponsiveContainer>;
}
